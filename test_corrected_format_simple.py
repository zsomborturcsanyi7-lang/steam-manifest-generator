...
for filename, (depot_id, manifest_id) in manifest_files.items():
    ...
    with open(os.path.join(output_dir, filename), "wb") as f:
        f.write(binary_data)
    print(f"  - {filename} letrehozva ({len(binary_data)} bytes)")
...
# Ellenőrizzük a főmappával való egyezést
print("\n4. Főmappával való összehasonlítás:")
main_folder = r"C:\Users\iga\Downloads\Plague_Inc__Evolved_T9"

if os.path.exists(main_folder):
    main_files = set(os.listdir(main_folder))
    # Távolítsuk el a GENERÁLÓÁLTAl mappát
    main_files.discard("GENERÁLÓÁLTAl")
    
    our_files = set(os.listdir(output_dir))
    
    if main_files == our_files:
        print("   ✓ A fájllista AZONOS!")
    else:
        print("   ✗ A fájllista KÜLÖNBÖZIK!")
        print(f"   Főmappa: {len(main_files)} fájl")
        print(f"   Mi: {len(our_files)} fájl")
        
        missing = main_files - our_files
        extra = our_files - main_files
        
        if missing:
            print(f"   Hiányzik tőlünk: {missing}")
        if extra:
            print(f"   Felesleges nálunk: {extra}")
else:
    print(f"   Főmappa nem található: {main_folder}")